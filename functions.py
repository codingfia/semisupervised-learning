...
def display_results_Zoepp(loss, property_corr, property_r2, args, header):
    property_corr = torch.mean(torch.cat(property_corr), dim=0).squeeze()
    property_r2 = torch.mean(torch.cat(property_r2), dim=0).squeeze()
    loss = torch.mean(torch.tensor(loss))
    corr_text = " | ".join([u"{:s}: {:.4f}".format(args.out_params[i], property_corr[i].squeeze()) for i in range(len(args.out_params))])
    r2_text =   " | ".join([u"{:s}: {:.4f}".format(args.out_params[i], property_r2[i].squeeze()) for i in range(len(args.out_params))])
    print("loss: {:.4f}\nCorrelation: {:s}\nr2 Coeff.  : {:s}".format(loss,corr_text,r2_text))
